...
Production-style deployment of the `platform-multi-tenant` fork via Docker Compose. The stack adds distributed flow execution (Arq + Redis) and multi-worker scale-out on top of the upstream single-container image.
...
└──────────────┘          └──┬──────────┬───┘
        ...
        │ default  │ │  high  │   │  webhooks  │  │   low     │
        │ (8/proc) │ │(4/proc)│   │ (16/proc)  │  │ (optional)│
        └──────────┘ └────────┘   └────────────┘  └───────────┘
```

The backend enqueues webhook/schedule/MCP-triggered flow runs to Arq; workers pick them up and write results back to Postgres. Without distributed execution, runs stay in-process on the API pod.
...
| **Redis 6+** | Required when `LANGFLOW_DISTRIBUTED_EXECUTION=true` (always, for the worker path). Also used when `LANGFLOW_CACHE_TYPE=redis` | Arq job broker, Langflow cache backend, concurrency/cancel signals between API and workers | `LANGFLOW_REDIS_URL` (workers), `LANGFLOW_REDIS_HOST/PORT/DB` (cache) |
...
Workers run the `langflow worker` CLI (Arq under the hood). The compose file ships three worker services, one per production queue:
...
Each process polls **one** queue (arq limitation). To serve multiple queues, run multiple processes — or replicate the service in compose:
...
| `LANGFLOW_WORKERS` | No | `1` | Uvicorn worker processes (unrelated to Arq workers) |
...
### Distributed execution (Arq + Redis)

| Var | Required | Default | Description |
|---|---|---|---|
| `LANGFLOW_DISTRIBUTED_EXECUTION` | Yes for worker path | `false` | Must be `true` on **both** backend and worker services. When false, runs stay in-process. |
| `LANGFLOW_REDIS_URL` | Yes for worker path | `redis://localhost:6379/0` | Arq broker DSN. Use the same value across API and all worker services. |
| `LANGFLOW_WORKER_CONCURRENCY` | No | `8` | Max in-flight jobs per worker process (override with `--concurrency`) |
| `LANGFLOW_ARQ_DEFAULT_QUEUE` | No | `runs:default` | Rename the default queue |
| `LANGFLOW_ARQ_HIGH_QUEUE` | No | `runs:high` | Rename the high-priority queue |
| `LANGFLOW_ARQ_LOW_QUEUE` | No | `runs:low` | Rename the low-priority queue |
| `LANGFLOW_ARQ_WEBHOOKS_QUEUE` | No | `webhooks` | Rename the webhook queue |
...
1. Run `alembic upgrade head` from the backend image (or from a one-shot job) before deploying new backend/worker images — schema is shared.
...
4. Frontend is static and can be deployed in any order relative to the backend, as long as the API contract is stable.
