...
3. To confirm that the container is serving your flows as expected, use the Langflow API to run a flow:
    ...
    2. Send a POST request to the [`/v1/run/$FLOW_ID`](/api-flows-run#run-flow) endpoint using the flow ID from the previous step .
        ...
        curl --request POST \
          ...
          --data '{
            "input_value": "Explain how the solar system formed.",
            "output_type": "chat",
            "input_type": "chat",
            "session_id": "solar_system_test_request"
        }'
        ```
        ...
        This command runs the **Basic Prompting** template flow.
        It provides chat input to the prompt, uses an optional custom `session_id`, and enables response streaming with `?stream=true`.
        ...
        This command uses response streaming because the model's response can be long.
