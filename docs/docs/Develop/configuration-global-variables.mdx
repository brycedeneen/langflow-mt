...
For example, `LANGFLOW_VARIABLES_TO_GET_FROM_ENVIRONMENT=CUSTOM_PROJECT_ID,CUSTOM_API_KEY` creates global variables named `CUSTOM_PROJECT_ID` and `CUSTOM_API_KEY` in Langflow's database.
...
For example, assume you have the following Langflow `.env` configuration, and your flow has a component that expects a `CUSTOM_API_KEY` global variable:

```text
LANGFLOW_FALLBACK_TO_ENV_VAR=True
CUSTOM_PROJECT_ID=your_project_id
CUSTOM_API_KEY=your_api_key
```

When you run the flow, if there is no global variable named `CUSTOM_API_KEY`, Langflow looks for an environment variable named `CUSTOM_API_KEY`.
In this example, Langflow uses the `CUSTOM_API_KEY` value from the `.env` to run the flow.