"""ADP consolidated US tax profile reads + writes.
...
async def _fetch_us_tax_profile(
    conn: ADPConnection,
    *,
    path: str,
    request_cache: RequestCache,
) -> dict[str, Any]:
    """GET US tax profile with cache peek before opening mTLS client."""
...
def build_us_tax_profiles_tools(
    connection: ADPConnection,
    request_cache: RequestCache,
    *,
    enable_mutations: bool = False,
) -> list[Tool]:
    """Build ADP US tax profile read + optional write tools."""
    conn = connection
    ...
    async def _get_worker_us_tax_profile(
        ...
        return await _fetch_us_tax_profile(conn, path=path, request_cache=request_cache)
