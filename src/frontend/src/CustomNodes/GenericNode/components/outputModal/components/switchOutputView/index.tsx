...
const SwitchOutputView: React.FC<SwitchOutputViewProps> = ({
  ...
  const results = ((type === "Outputs"
    ? flowPoolNode?.data?.outputs?.[outputName]
    : flowPoolNode?.data?.logs?.[outputName]) ?? {}) as Partial<
    OutputLogType & LogsLogType
  >;
  const resultType = results?.type;
  // resultMessage is union-typed (string | ErrorLogType | record | array | null)
  // — use `any` locally so the existing duck-typing logic below stays readable.
  let resultMessage: any = results?.message ?? {};
