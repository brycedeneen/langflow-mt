...
async def execute_run(
    ...
    *,
    ...
    async with session_factory() as session:
        ...
        org_limit = org.runs_max_concurrent
        org_priority_tier = org.runs_priority_tier
        flow_captured = flow
    ...
    try:
        ...
        async with session_factory() as session:
            run = await session.get(FlowRun, run_uuid)
            if run is None:
                logger.warning(f"[run={run_id}] FlowRun disappeared before RUNNING transition; dropping job")
                return
        ...
        logger.info(
            f"[run={run_id}] RUNNING flow={flow_captured.id} ({flow_captured.name!r}) org={org_id_captured} "
            f"trigger={triggered_by_captured} attempt={run.attempt}"
        )

        from langflow.services.runs.metrics import ACTIVE_RUNS
        ACTIVE_RUNS.labels(organization_id=str(org_id_captured)).inc()
        active_runs_inc = True
...
def _suppress():
    """Drain a possibly-cancelled task without raising routine exceptions.

    Re-raises KeyboardInterrupt / SystemExit so process-control signals are not lost.
    """
